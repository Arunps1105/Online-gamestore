...
// Handle download insert
if (isset($_GET['gid']) && isset($_GET['game'])) {
    $gid = $_GET['gid'];
    $uid = $_SESSION['uid'];

    $insQry = "INSERT INTO tbl_download(download_date, game_id, user_id) VALUES (CURDATE(), '$gid', '$uid')";
    if ($Con->query($insQry)) {
    // get game file path
    $fileQry = "SELECT game_file FROM tbl_game WHERE game_id = '$gid'";
    $fileRes = $Con->query($fileQry);
    $fileData = $fileRes->fetch_assoc();
    $filePath = "../Assets/Files/Developer/Game/" . $fileData['game_file'];
...
if (file_exists($filePath)) {
    ...
    echo "<script>alert('File not found!'); window.location='Viewgamedetails.php?gid=$gid';</script>";
}

}else {
        echo "<script>alert('Failed to record download');</script>";
    }
}
?>
