...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $chatId = $_GET['chat_id'];
    $delQry = "DELETE FROM tbl_chat WHERE chat_id = '$chatId' AND from_developer_id = '$did' AND community_id = '$communityId'";
    echo $Con->query($delQry) ? "Message deleted" : "Deletion failed";
    exit;
}

if (isset($_GET['action']) && $_GET['action'] == 'clear') {
    $delQry = "DELETE FROM tbl_chat WHERE community_id = '$communityId' AND from_developer_id = '$did'";
...
if ($checkResult->num_rows == 0 && ($msg !== '' || $filePath !== '')) {
    $insQry = "INSERT INTO tbl_chat (from_user_id, from_developer_id, chat_content, chat_date, chat_file, community_id) 
               VALUES (0, '$did', '$msg', NOW(), '$filePath', '$communityId')";
